      ...
      {opportunities.length === 0 ? (
        ...
        <div className="space-y-6">
          <p className="text-gray-400">
            Found {opportunities.length} potential arbitrage opportunit{opportunities.length > 1 ? 'ies' : 'y'}. This
            occurs when odds from different bookmakers are misaligned, allowing for a guaranteed profit by betting on
            all outcomes.
          </p>
            ...
            <div key={opp.matchId} className="bg-gray-700/50 p-4 rounded-lg border border-green-500/50">
              <div className="flex justify-between items-start mb-3">
                <div>
                  ...
                  <p className="text-xs text-gray-400">{new Date(opp.commenceTime).toLocaleString()}</p>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-center">
                {opp.outcomes.map((outcome) => (
                  <div key={outcome.name} className="bg-gray-800 p-3 rounded-md">
                    <p className="text-sm font-semibold text-white truncate" title={outcome.name}>
                      {outcome.name}
                    </p>
              ...
              <div className="mt-4 pt-4 border-t border-gray-600">
                ...
                <p className="text-xs text-gray-400 mb-3">
                  For a total stake of £100, distribute your bets as follows to guarantee a profit of £
                  {opp.profitPercentage.toFixed(2)}.
                </p>
                <ul className="space-y-2 text-sm">
                  {opp.outcomes.map((outcome) => (
                    <li key={outcome.name} className="flex items-center bg-gray-800 p-2 rounded-md">
                      <span className="font-bold text-green-400 w-20">
                        £{(outcome.stakePercentage * 100).toFixed(2)}
                      </span>
                      <span className="text-gray-300">
                        on <span className="font-semibold text-white">{outcome.name}</span>
                      </span>
