...
const BarDisplay: React.FC<BarDisplayProps> = ({ teamName, winProbability, color }) => {
  ...
  return (
    <div className="flex items-center gap-4 text-sm">
      <div className="w-1/3 sm:w-1/4 font-medium text-gray-300 truncate text-right">{teamName}</div>
      <div className="w-2/3 sm:w-3/4 flex items-center gap-2">
        ...
        <div className={`w-16 text-left font-semibold ${textColorClass}`}>{(winProbability * 100).toFixed(1)}%</div>
      </div>
    </div>
  );
};
    ...
    <CollapsibleSection title="Match Win Probabilities" defaultOpen={false}>
      <div className="space-y-4">
        {matchPairs.map((pair, index) => (
          <div
            key={`${pair.favorite.teamName}-${index}`}
            className="space-y-1.5 border-b border-gray-700/50 last:border-b-0 pb-4 last:pb-0"
          >
            <BarDisplay teamName={pair.favorite.teamName} winProbability={pair.favorite.winProbability} color="green" />
            <BarDisplay teamName={pair.underdog.teamName} winProbability={pair.underdog.winProbability} color="red" />
          </div>
        ))}
      </div>
    </CollapsibleSection>
