  ...
  const stats = useMemo(() => {
    ...
    data.forEach((match) => {
      ...
      const oddsByOutcome = {
        ...
        draw: new Map<string, number>(),
      };
      const bestMatchOdds = { home: { price: 0 }, away: { price: 0 }, draw: { price: 0 } };

      match.bookmakers.forEach((bookie) => {
        bookmakers.add(bookie.title);
        if (!bestOddsProvider.has(bookie.title)) {
          bestOddsProvider.set(bookie.title, { home: 0, away: 0, draw: 0 });
        }
      ...
      if (homeStdDev > marketDisagreement.stdDev)
        marketDisagreement = { match: matchTitle, outcome: 'Home Win', stdDev: homeStdDev };
      const awayStdDev = stdDev(Array.from(oddsByOutcome.away.values()));
      if (awayStdDev > marketDisagreement.stdDev)
        marketDisagreement = { match: matchTitle, outcome: 'Away Win', stdDev: awayStdDev };
    ...
    const bookmakerVigAverages = Array.from(bookmakerVigorish.entries())
      .map(([name, { totalVig, count }]) => ({
        name,
    ...
    return {
      ...
      uniqueDates: Array.from(datesByIsoDay.values())
        .sort((a, b) => a.ts - b.ts)
        .map((d) => d.label),
      bestArbitrage,
      ...
      bestOddsProvider: Array.from(bestOddsProvider.entries()).sort(
        (a, b) => b[1].home + b[1].away + b[1].draw - (a[1].home + a[1].away + a[1].draw)
      ),
    };
  }, [data]);
      ...
      <div className="space-y-6">
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            />
            <StatCard label="API Quota Used" value={quotaUsage?.requestsUsed ?? 'N/A'} color="text-blue-400" />
            <StatCard
              label="API Quota Left"
              value={quotaUsage?.requestsRemaining ?? 'N/A'}
              color={
                quotaUsage?.requestsRemaining !== null &&
                quotaUsage?.requestsRemaining !== undefined &&
                quotaUsage.requestsRemaining < 20
                  ? 'text-yellow-400'
                  : 'text-green-400'
              }
            />
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <AdvancedStat label="Best Arbitrage Opportunity">
              {stats.bestArbitrage ? (
                <>
                  <span className="font-bold text-green-400">{stats.bestArbitrage.profitPercentage.toFixed(2)}%</span>{' '}
                  on {stats.bestArbitrage.matchTitle}
                </>
              ) : (
                'None found'
              )}
            ...
            <AdvancedStat label="Biggest Market Disagreement">
              {stats.marketDisagreement ? (
                <>
                  On <span className="font-bold">{stats.marketDisagreement.outcome}</span> for{' '}
                  {stats.marketDisagreement.match}
                </>
              ) : (
                'N/A'
              )}
