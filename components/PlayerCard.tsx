...
const PlayerCard: React.FC<PlayerCardProps> = ({ player, allTeams, onNameChange, onAddPick, onRemovePick }) => {
  ...
  return (
    <div className="rainbow-snake-border bg-gray-800 rounded-xl p-4 border border-gray-600 hover:border-green-500 transition-colors duration-300 flex flex-col gap-4">
        ...
        <div className="bg-gray-900/50 p-3 rounded-md min-h-[7.5rem] border border-gray-600 flex items-center justify-center">
          {player.suggestion ? (
            <div className="text-center w-full">
              ...
              <div className="text-xs text-gray-400 mt-1">vs {player.suggestion.vsTeam}</div>
              ...
              <div className="mt-2 text-sm">
                ...
                <span className={`font-bold ${getSuggestionColor(player.suggestion.type)}`}>
                  {isFinite(player.suggestion.odds) ? player.suggestion.odds.toFixed(2) : 'N/A'}
                </span>
              </div>
            </div>
          ) : (
      ...
      <div>
        <label htmlFor={`team-select-${player.id}`} className="sr-only">
          Add a team
        </label>
        <select
