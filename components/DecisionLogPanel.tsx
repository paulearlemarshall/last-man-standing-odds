...
const DecisionLogPanel: React.FC<DecisionLogPanelProps> = ({ logs }) => {
  ...
  const getTypeStyles = (type: DecisionLogEntry['type']) => {
    switch (type) {
      case 'success':
        return 'text-green-400 border-l-2 border-green-500 bg-green-900/10';
      case 'warning':
        return 'text-yellow-400 border-l-2 border-yellow-500 bg-yellow-900/10';
      case 'error':
        return 'text-red-400 border-l-2 border-red-500 bg-red-900/10';
      case 'process':
        return 'text-blue-300 border-l-2 border-blue-500 bg-blue-900/10';
      default:
        return 'text-gray-300 border-l-2 border-gray-500';
    }
  };
        ...
        <div className="bg-gray-900 rounded-lg p-4 font-mono text-sm h-96 overflow-y-auto">
          {logs.map((log) => (
            <div key={log.id} className={`mb-2 pl-3 py-1 ${getTypeStyles(log.type)}`}>
              ...
              {log.details && <div className="text-xs text-gray-500 mt-1 pl-1">{log.details}</div>}
            </div>
          ))}
        </div>
      </div>
