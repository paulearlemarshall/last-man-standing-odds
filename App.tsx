...
const App: React.FC = () => {
  ...
  const [players, setPlayers] = useState<Player[]>([{ id: 1, name: 'Player 1', previousPicks: [], suggestion: null }]);
  ...
  useEffect(() => {
    ...
    fetchSportsDirectory(abortController.signal)
      ...
      .catch((directoryError) => {
        ...
        setSportsDirectoryError(
          directoryError instanceof Error ? directoryError.message : 'Could not load active sports.'
        );
      });
        ...
        <div className="flex flex-col items-center justify-center gap-4 py-4 px-6 mb-8 bg-gray-800/50 border-y-2 border-green-500/30 rounded-lg">
          <div className="flex flex-wrap items-center justify-center gap-4">
            <div className="relative">
              <select
                value={currentSportClass}
                onChange={handleSportChange}
                className="px-4 py-2 bg-slate-700 hover:bg-slate-600 rounded-md text-white border-none focus:ring-2 focus:ring-green-500 outline-none cursor-pointer appearance-none pr-8 font-bold"
              >
                {Object.keys(sportsDirectory).map((sport) => (
            ...
            <div className="relative">
              <select
                ...
                onChange={(e) => {
                  const league = sportsDirectory[currentSportClass].find((l) => l.key === e.target.value);
                  if (league) setCurrentLeague(league);
                }}
                className="px-4 py-2 bg-slate-700 hover:bg-slate-600 rounded-md text-white border-none focus:ring-2 focus:ring-green-500 outline-none cursor-pointer appearance-none pr-8"
              >
                {sportsDirectory[currentSportClass].map((league) => (
            ...
            <div className="flex bg-slate-700 rounded-md p-1">
              {ALL_REGIONS.map((region) => (
                <button
                  ...
                  className={`px-3 py-1 rounded-sm text-sm font-bold transition-colors uppercase ${selectedRegions.includes(region) ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-400 hover:text-white hover:bg-slate-600'}`}
                >
                  {region}
                </button>
              ))}
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-slate-600 hover:bg-slate-700 rounded-md disabled:opacity-50 transition-colors"
            >
              <svg
            ...
            <button
              ...
              disabled={loading || !matchWeekends.length}
              className="px-4 py-2 bg-green-600 hover:bg-green-700 rounded-md disabled:opacity-50 transition-colors"
            >
              Suggest
            </button>
            <button
          ...
          <div className="flex flex-col items-center gap-1">
            {lastRefreshTime && (
              <p className="text-xs text-gray-500">Last refreshed: {lastRefreshTime.toLocaleTimeString()}</p>
            )}
            {shareMessage && <p className="text-xs text-blue-300">{shareMessage}</p>}
            {sportsDirectoryError && (
              <p className="text-xs text-yellow-300">Using fallback sports list: {sportsDirectoryError}</p>
            )}
          </div>
        </div>
        <main className="space-y-8">
          <PlayerManager
            ...
            onNameChange={(id, name) =>
              setPlayers((p) => p.map((player) => (player.id === id ? { ...player, name } : player)))
            }
            onAddPick={(id, team) =>
              setPlayers((p) =>
                p.map((player) =>
                  player.id === id ? { ...player, previousPicks: [...player.previousPicks, team].sort() } : player
                )
              )
            }
            onRemovePick={(id, team) =>
              setPlayers((p) =>
                p.map((player) =>
                  player.id === id
                    ? { ...player, previousPicks: player.previousPicks.filter((t) => t !== team) }
                    : player
                )
              )
            }
          ...
          {!loading && !error && matchWeekends.length > 0 && (
            <>
              {matchWeekends.map((weekend) => (
                <MatchWeekendView key={weekend.id} weekend={weekend} bookmakers={allBookmakers} />
              ))}
